...
#define AssignCommand(obj, act) \
  do { \
    object _temp = nwn_GetCommandObject(); \
    nwn_SetCommandObject(obj); \
    act; \
    nwn_SetCommandObject(_temp); \
  } while (0)

#define DelayCommand(delay, act) \
    nwn_DelayCommand(nwn_GetCommandObjectId(), delay, "$nwcpp", \
                     ScriptManager::RegisterScriptSituation(std::function<void()>([=] () { act; })))
#define ActionDoCommand(act) \
    nwn_ActionDoCommand(nwn_GetCommandObjectId(), \
                        "$nwcpp", \
                        ScriptManager::RegisterScriptSituation(std::function<void()>([=] () { act; })))

/**
  * DelayCommandEx and ActionDoCommandEx are extensions to allow passing in your
  * own lambda captures.
  *
  * Note: It would probably be unwise capture any variable except by value.
  */

inline void ActionDoCommandEx(const std::function<void()>& act)
{
    nwn_ActionDoCommand(nwn_GetCommandObjectId(),
                        "$nwcpp",
                        ScriptManager::RegisterScriptSituation(act));
}

inline void DelayCommandEx(float delay, const std::function<void()>& func)
{
    nwn_DelayCommand(nwn_GetCommandObjectId(), delay, "$nwcpp",
                     ScriptManager::RegisterScriptSituation(func));
...
// * No return value, but if an error occurs the log file will contain
//   "MoveToPoint failed."
void ActionMoveToLocation(const location&  lDestination, int bRun = FALSE);
...
object CreateItemOnObject(const string& sItemTemplate, object oTarget = OBJECT_SELF,
                          int nStackSize = 1, const string& sNewTag = "");
...
object GetNearestCreature(int nFirstCriteriaType, int nFirstCriteriaValue,
                          object oTarget = OBJECT_SELF, int nNth = 1, int nSecondCriteriaType = -1,
                          int nSecondCriteriaValue = -1, int nThirdCriteriaType = -1,  int nThirdCriteriaValue = -1);

// Add a speak action to the action subject.
// - sStringToSpeak: String to be spoken
// - nTalkVolume: TALKVOLUME_*
void ActionSpeakString(const string& sStringToSpeak, int nTalkVolume = TALKVOLUME_TALK);
...
void ActionCastSpellAtObject(int nSpell, object oTarget, int nMetaMagic = METAMAGIC_ANY,
                             int bCheat = FALSE, int nDomainLevel = 0, int nProjectilePathType = PROJECTILE_PATH_TYPE_DEFAULT,
...
// * Return value on error: ""
string InsertString(string sDestination, const string& sString, int nPosition);
...
// * Return value on error: -1
int FindSubString(const string& sString, const string& sSubString, int nStart = 0);
...
effect EffectSummonCreature(const string& sCreatureResref, int nVisualEffectId = VFX_NONE,
                            float fDelaySeconds = 0.0f, int nUseAppearAnimation = 0);
...
int FortitudeSave(object oCreature, int nDC, int nSaveType = SAVING_THROW_TYPE_NONE,
...
// Note: If used within an Area of Effect Object Script (On Enter, OnExit, OnHeartbeat), you MUST pass
// GetAreaOfEffectCreator() into oSaveVersus!!
int ReflexSave(object oCreature, int nDC, int nSaveType = SAVING_THROW_TYPE_NONE,
               object oSaveVersus = OBJECT_SELF);
...
// Note: If used within an Area of Effect Object Script (On Enter, OnExit, OnHeartbeat), you MUST pass
// GetAreaOfEffectCreator() into oSaveVersus!!
int WillSave(object oCreature, int nDC, int nSaveType = SAVING_THROW_TYPE_NONE,
             object oSaveVersus = OBJECT_SELF);
...
// Return value on error: OBJECT_INVALID
object GetFirstObjectInShape(int nShape, float fSize, const location&  lTarget,
                             int bLineOfSight = FALSE, int nObjectFilter = OBJECT_TYPE_CREATURE, vec3 vOrigin = vec3());
...
// Return value on error: OBJECT_INVALID
object GetNextObjectInShape(int nShape, float fSize, const location&  lTarget,
                            int bLineOfSight = FALSE, int nObjectFilter = OBJECT_TYPE_CREATURE, vec3 vOrigin = vec3());
...
effect EffectAreaOfEffect(int nAreaEffectId, const string& sOnEnterScript = "",
                          const string& sHeartbeatScript = "", const string& sOnExitScript = "");
...
void SetListenPattern(object oObject, const string& sPattern, int nNumber = 0);
...
object GetObjectByTag(const string& sTag, int nNth = 0);
...
void SetAreaTransitionBMP(int nPredefinedAreaTransition,
                          const string& sCustomAreaTransitionBMP = "");
...
void ApplyEffectToObject(int nDurationType, const effect& eEffect, object oTarget,
                         float fDuration = 0.0f);
...
// - sStringToSpeak
// - nTalkVolume: TALKVOLUME_*
void SpeakString(const string& sStringToSpeak, int nTalkVolume = TALKVOLUME_TALK);
...
object GetNearestCreatureToLocation(int nFirstCriteriaType, int nFirstCriteriaValue,
                                    const location&  lLocation, int nNth = 1, int nSecondCriteriaType = -1,
                                    int nSecondCriteriaValue = -1, int nThirdCriteriaType = -1, int nThirdCriteriaValue = -1);
...
// - nNth
// * Return value on error: OBJECT_INVALID
object GetNearestObject(int nObjectType = OBJECT_TYPE_ALL, object oTarget = OBJECT_SELF,
                        int nNth = 1);
...
// - nNth
// * Return value on error: OBJECT_INVALID
object GetNearestObjectToLocation(int nObjectType, const location&  lLocation, int nNth = 1);

// Get the nth Object nearest to oTarget that has sTag as its tag.
// * Return value on error: OBJECT_INVALID
object GetNearestObjectByTag(const string& sTag, object oTarget = OBJECT_SELF, int nNth = 1);
...
void   ActionCastSpellAtLocation(int nSpell, const location&  lTargetLocation,
...
object CreateObject(int nObjectType, const string& sTemplate, const location& lLocation,
                    int bUseAppearAnimation = FALSE, const string& sNewTag = "");
...
//   event will be used
int BeginConversation(const string& sResRef = "", object oObjectToDialog = OBJECT_INVALID);
...
object GetFirstInPersistentObject(object oPersistentObject = OBJECT_SELF,
                                  int nResidentObjectType = OBJECT_TYPE_CREATURE, int nPersistentZone = PERSISTENT_ZONE_ACTIVE);
...
//   PERSISTENT_ZONE_FOLLOW, but this is no longer used.]
// * Returns OBJECT_INVALID if no object is found.
object GetNextInPersistentObject(object oPersistentObject = OBJECT_SELF,
                                 int nResidentObjectType = OBJECT_TYPE_CREATURE, int nPersistentZone = PERSISTENT_ZONE_ACTIVE);
...
void DeleteLocalLocation(object oObject, const string& sVarName);
...
void ActionUseSkill(int nSkill, object oTarget, int nSubSkill = 0,
...
// - nSaveType: SAVING_THROW_TYPE_*
// - oSaveVersus
int GetReflexAdjustedDamage(int nDamage, object oTarget, int nDC,
                            int nSaveType = SAVING_THROW_TYPE_NONE, object oSaveVersus = OBJECT_SELF);
...
int GetCreatureHasTalent(const talent&  tTalent, object oCreature = OBJECT_SELF);
...
effect VersusAlignmentEffect(const effect& eEffect, int nLawChaos = ALIGNMENT_ALL,
...
void ActionMoveAwayFromLocation(const location& lMoveAwayFrom, int bRun = FALSE,
...
//   everyone in the world
void RemoveJournalQuestEntry(const string& szPlotID, object oCreature, int bAllPartyMembers = TRUE,
...
void SendMessageToPC(object oPlayer, const string& szMessage);
...
void ActionForceMoveToLocation(const location& lDestination, int bRun = FALSE,
                               float fTimeout = 30.0f);

// Force the action subject to move to oMoveTo.
void ActionForceMoveToObject(object oMoveTo, int bRun = FALSE, float fRange = 1.0f,
                             float fTimeout = 30.0f);
...
void SetIsTemporaryFriend(object oTarget, object oSource = OBJECT_SELF, int bDecays = FALSE,
...
void SetIsTemporaryEnemy(object oTarget, object oSource = OBJECT_SELF, int bDecays = FALSE,
                         float fDurationInSeconds = 180.0f);
...
// (faction reputation + total personal reputation) < REPUTATION_TYPE_FRIEND.
void SetIsTemporaryNeutral(object oTarget, object oSource = OBJECT_SELF, int bDecays = FALSE,
                           float fDurationInSeconds = 180.0f);
...
//   in the string.
void SpeakOneLinerConversation(const string& sDialogResRef = "",
...
// Activate oItem.
event EventActivateItem(object oItem, const location& lTarget, object oTarget = OBJECT_INVALID);
...
void ActivatePortal(object oTarget, const string& sIPaddress = "", const string& sPassword = "",
                    const string& sWaypointTag = "", int bSeemless = FALSE);
...
effect EffectDisappearAppear(const location& lLocation, int nAnimation = 1);
...
// - oTarget
// - nProjectilePathType: PROJECTILE_PATH_TYPE_*
void ActionCastFakeSpellAtObject(int nSpell, object oTarget,
...
// - lTarget
// - nProjectilePathType: PROJECTILE_PATH_TYPE_*
void ActionCastFakeSpellAtLocation(int nSpell, const location& lTarget,
                                   int nProjectilePathType = PROJECTILE_PATH_TYPE_DEFAULT);
...
// - sCreatureTemplate3
// - sCreatureTemplate4
effect EffectSwarm(int nLooping, const string& sCreatureTemplate1,
                   const string& sCreatureTemplate2 = "", const string& sCreatureTemplate3 = "",
                   const string& sCreatureTemplate4 = "");
...
// - oCreature
void SetStandardFactionReputation(int nStandardFaction, int nNewReputation,
...
void PopUpDeathGUIPanel(object oPC, int bRespawnButtonEnabled = TRUE,
                        int bWaitForHelpButtonEnabled = TRUE, int nHelpStringReference = 0, const string& sHelpString = "");
...
// The var name must be unique across the entire database, regardless of the variable type.
// If you want a variable to pertain to a specific player in the game, provide a player object.
void SetCampaignInt(const string& sCampaignName, const string& sVarName, int nInt,
                    object oPlayer = OBJECT_INVALID);
...
// The var name must be unique across the entire database, regardless of the variable type.
// If you want a variable to pertain to a specific player in the game, provide a player object.
void SetCampaignVector(const string& sCampaignName, const string& sVarName, vec3 vVector,
                       object oPlayer = OBJECT_INVALID);
...
void SetCampaignLocation(const string& sCampaignName, const string& sVarName,
                         const location& locLocation, object oPlayer = OBJECT_INVALID);
...
// The var name must be unique across the entire database, regardless of the variable type.
// If you want a variable to pertain to a specific player in the game, provide a player object.
float GetCampaignFloat(const string& sCampaignName, const string& sVarName,
                       object oPlayer = OBJECT_INVALID);
...
// The var name must be unique across the entire database, regardless of the variable type.
// If you want a variable to pertain to a specific player in the game, provide a player object.
int GetCampaignInt(const string& sCampaignName, const string& sVarName,
                   object oPlayer = OBJECT_INVALID);
...
// The var name must be unique across the entire database, regardless of the variable type.
// If you want a variable to pertain to a specific player in the game, provide a player object.
vec3 GetCampaignVector(const string& sCampaignName, const string& sVarName,
                       object oPlayer = OBJECT_INVALID);
...
// The var name must be unique across the entire database, regardless of the variable type.
// If you want a variable to pertain to a specific player in the game, provide a player object.
location GetCampaignLocation(const string& sCampaignName, const string& sVarName,
                             object oPlayer = OBJECT_INVALID);
...
// The var name must be unique across the entire database, regardless of the variable type.
// If you want a variable to pertain to a specific player in the game, provide a player object.
string GetCampaignString(const string& sCampaignName, const string& sVarName,
                         object oPlayer = OBJECT_INVALID);
...
object CopyObject(object oSource, const location& locLocation, object oOwner = OBJECT_INVALID,
...
// from the game.
void DeleteCampaignVariable(const string& sCampaignName, const string& sVarName,
                            object oPlayer = OBJECT_INVALID);
...
object RetrieveCampaignObject(const string& sCampaignName, const string& sVarName,
                              const location& locLocation, object oOwner = OBJECT_INVALID, object oPlayer = OBJECT_INVALID);
...
void AddItemProperty(int nDurationType, itemproperty ipProperty, object oItem,
                     float fDuration = 0.0f);
...
//                           traps.2da).
object CreateTrapAtLocation(int nTrapType, const location& lLocation, float fSize = 2.0f,
                            const string& sTag = "", int nFaction = STANDARD_FACTION_HOSTILE,
                            const string& sOnDisarmScript = "", const string& sOnTrapTriggeredScript = "");
...
void SetDescription(object oObject, const string& sNewDescription = "",
                    int bIdentifiedDescription = TRUE);
...
#define TINYFORMAT_ERROR(reason) \
    nx_log(NX_LOG_ERROR, 0, "ERROR: tinyformat %s", reason)
