 ...
 * This will only work inside nwnx_effect callback scripts. Returns -1 on error.
 */
int GetCustomEffectTickRate(effect eff);
 ...
 */
void SetCustomEffectTickRate(effect eff, int value);
...
int GetCustomEffectTickRate(effect eff)
{
    // Note this CANNOT be replaced by a call to GetEffectInteger.
    SetLocalString(GetModule(), "NWNX!STRUCTS!GETINTEGER", "20          ");
    return StringToInt(GetLocalString(GetModule(), "NWNX!STRUCTS!GETINTEGER"));
}

void SetCustomEffectTickRate(effect eff, int value)
{
    // Note this CANNOT be replaced by a call to SetEffectInteger.
    SetLocalString(GetModule(), "NWNX!STRUCTS!SETINTEGER", "20 " + IntToString(value));
}
