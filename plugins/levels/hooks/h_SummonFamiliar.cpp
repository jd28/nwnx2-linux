...
void Hook_SummonFamiliar(CNWSCreature *cre) {
    ...
    int32_t level = nwn_GetLevelByClass(cre->cre_stats, CLASS_TYPE_SORCERER);
    level += nwn_GetLevelByClass(cre->cre_stats, CLASS_TYPE_WIZARD);
    if ( level <= 0 ) { 
        return;
    }
    else if ( level > 40 ) {
    ...
    uint32_t assoc_id = CNWSCreature__GetAssociateId(cre, 3, 1);
    ...
    int familiar_type = cre->cre_stats->cs_famil_type;
    const char *base = nwn_Get2daString(fam2da, "BASERESREF", familiar_type);
    if ( level > 9 ) {
    ...
    levels.Log(3, "SummonFamiliar Resref: |%s|\n", resref.resref);

    char *name = new char[250];
    snprintf(name, 250, "%s", cre->cre_stats->cs_famil_name.text);

    levels.Log(3, "SummonFamiliar Name: |%s|\n", name);

    CNWSCreature__SummonAssociate(cre, resref, &name, 3);
    cre->cre_summoned_famil = 1;
}
